...
> **v1.12 변경 이력 (2026-03-16)**:  
> - §2.1 점주 가입: 매장 정보 UI **매장명·업종 하단 배치** 및 `store.floorCount` 미수집 시 기본값 전달 안내

> **v1.13 변경 이력 (2026-03-16)**:  
> - §3.7 **개별 매장 대시보드** `GET /dashboard/stores/:storeId` 신설: 응답에 **`phone`(매장 연락처)** 포함, `StoreDashboard` UI와 정합  
> - 부록 라우트 목록에 동 엔드포인트 반영

> **v1.14 변경 이력 (2026-04-14)**:  
> - §9.4.3 **댐퍼/팬 자동제어 기본값** (시스템관리 기준수치 탭): 집진기별 `damperAutoSettings` 배열, `GET/PUT /system/thresholds` 권장 및 분리 엔드포인트 대안, 시스템 시드 기본값(850 CMH / 3.5 m/s), §8.4 `damper_auto_settings` 매핑 안내  
> - 기존 §9.4.3 장비 모델 → **§9.4.4**로 번호 이동  
> - §6.4에 §9.4.3과의 **역할 구분** 비고 추가  
> - 부록 화면→API 매핑(행 20)에 기준수치 묶음·댐퍼/팬 기본값 반영
---
